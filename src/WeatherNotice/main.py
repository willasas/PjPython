...
# 扫码登录微信，如果在Linux环境中运行，加一个参数 bot = Bot(console_qr=-2,cache_path=True)
bot = Bot(cache_path=True)


def get_weather():
    url = 'http://v.juhe.cn/weather/index?cityname=深圳&key=xxx'  # 城市名cityname和key值换成自己的
...
while True:   # ！！！调试时记得先把while True注释掉，不然会一直重复发送失败，一天限制100次调用的，成功后再加上注释
    try:
        ...
        # 单独私发微信
        # my_friend = bot.friends().search(u'诗酒云梦')[0]  # 此处是对方自己的昵称，不是微信号，也不是你的备注。
    ...
    # 异常处理，发送失败，发送提醒消息给自己
        ...
        my_friend = bot.friends().search(u'诗酒云梦')[
